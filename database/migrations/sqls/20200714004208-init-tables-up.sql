...
VALUES 
    ...
    ("2020-03-03","2020-03-03", 1,2,2,250.00,3250.00,now(),now());
...
CREATE PROCEDURE `create_listing`(IN p_user_id INT,IN p_name VARCHAR(255),
    ...
    IN p_property_type VARCHAR(20), IN p_room_type VARCHAR(20), 
    IN p_address VARCHAR(255), IN p_latitude DECIMAL(10,8), 
    IN p_longitude DECIMAL(11,8), IN p_bed_count INT, IN p_bathroom_count INT,
    IN p_max_guest INT, IN p_price_by_night DECIMAL(10,2))
BEGIN
	INSERT INTO `listings` (`user_id`,`name`, `description`, `property_type`, 
	`room_type`, `address`,`latitude`,`longitude`,`bed_count`,`bathroom_count`,
	`max_guest`, `price_by_night`,`created_at`, `updated_at`)
    VALUES (p_user_id, p_name,p_description, p_property_type, p_room_type,
...
BEGIN
    ...
    `listing_id`, `price_per_day`, `price_for_stay`, `created_at`,`updated_at`)
    VALUES (p_start_date,p_end_date,p_is_approved,p_user_id,
    p_listing_id, p_price_per_day,p_price_for_stay, now(), now());
END;
...
BEGIN
    ...
    `created_at`,
    `updated_at`)
	VALUES(p_user_id, p_listing_id, p_booking_id, p_rating_num, 
    p_content, now(),now());
END;
